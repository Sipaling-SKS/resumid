...
actor Resumid {
    ...
    public shared (msg) func AnalyzeResumeV2(fileName : Text, resumeContent : Text, jobTitle : Text) : async ?GeminiTypes.AnalyzeStructureResponse {
      ...
      switch (analyzeResult) {
        ...
        case (?result) {
          ...
          // Konversi konten analisis ke tipe internal HistoryTypes.ContentItem
          ...
          // Konversi conclusion dan summary
          ...
          let history : HistoryTypes.History = {
            userId = userId;
            historyId = "history-" # debug_show(Time.now()); // atau gunakan UUID generator jika ada
            fileName = fileName;
            jobTitle = jobTitle;
          ...
          // Simpan ke dalam HashMap histories
          switch (histories.get(userId)) {
            case null {
              histories.put(userId, [history]);
            };
            case (?existing) {
              histories.put(userId, Array.append([history], existing));
            };
          };
  ...
  let userId = Principal.toText(msg.caller);
  HistoryServices.addDummyHistoriesSync(histories, userId); // ✅ betulkan argumen
