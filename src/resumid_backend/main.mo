...
actor Resumid {
 ...
 public shared (msg) func addHistory(input : HistoryTypes.AddHistoryInput) : async { #ok : HistoryTypes.History; #err : Text } {
  let userId = Principal.toText(msg.caller);
  return await HistoryServices.addHistory(histories, userId, input);
};
  ...
  let userId = Principal.toText(msg.caller);
  return await HistoryServices.getHistoriesPaginated(histories, userId, page, pageSize, sortBys, filterBys, globalFilter);
};
