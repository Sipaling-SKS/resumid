    ...
    let allHistories = switch (histories.get(userId)) {
        ...
        case (?filters) {
            ...
            h := Array.filter<HistoryTypes.History>(h, func(hist) {
                ...
                case ("historyId") {
                    Text.contains(hist.historyId, #text keyword)
                };
                case ("content") {
                    let lower = Text.toLowercase(keyword);
                    Option.isSome(Array.find(hist.content, func(c: HistoryTypes.ContentItem) : Bool {
                    return Text.contains(Text.toLowercase(c.title), #text lower)
