...
const resolver: Resolver<EducationFormValues> = async (values) => {
  ...
  if (!values.isNew) {
    if (!values.institution?.trim()) {
      errors.company = { type: "required", message: "Institution is required." };
    }
    if (!values.degree?.trim()) {
      errors.position = { type: "required", message: "Degree is required." };
    }
    if (!values.period?.start) {
      errors["period.start"] = { type: "required", message: "Start date is required." };
    } else {
      // Validate end >= start when end is provided
...
export function EducationDialog({
  ...
  const { mutateAsync: addEducation, isPending: isAdding } = useMutation({
    ...
    onError: (error) => {
      toast({
        title: "Error",
        description: `Error adding education: ${(error as any)?.message || "Something happened"}`,
        variant: "destructive",
      });
    },
  });
  ...
  const { mutateAsync: updateEducation, isPending: isUpdating } = useMutation({
    ...
    onError: (error, _vars, ctx) => {
      ...
      toast({
        title: "Error",
        description: `Error updating education: ${(error as any)?.message || "Something happened"}`,
        variant: "destructive",
      });
    },
    onSuccess: () => {
  ...
  const { mutateAsync: removeEducation, isPending: isRemoving } = useMutation({
    ...
    onError: (error, _vars, ctx) => {
      ...
      toast({
        title: "Error",
        description: `Error removing education: ${(error as any)?.message || "Something happened"}`,
        variant: "destructive",
      });
    },
    onSuccess: () => {
