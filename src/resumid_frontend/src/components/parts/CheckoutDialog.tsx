...
export default function CheckoutDialog({ open, onOpenChange, plan }: CheckoutDialogProps) {
  ...
  const { isAuthenticated, login, resumidActor } = useAuth();
  ...
  const { mutate: transaction, isPending: isLoading } = useMutation({
    ...
    onError: (error) => {
      console.error(error);
      toast({ title: "Failed to checkout", description: error.message ? error.message : "Please try again later", variant: "destructive" });
    },
    onSuccess: () => {
      toast({
        title: "Payment successful",
        description: `${plan?.tokens} tokens added to your account.`,
      });
      onOpenChange(false);
      setTimeout(() => {
        navigate('/wallet');
  ...
  return (
    <>
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="inline-flex items-center gap-2">
                    <img src={ICW} alt="ICP" className="w-4 h-4" />
                    <span className="font-inter text-[16px] text-gray-500">{transactionFee ?? defaultFee}</span>
                  </div>
                </div>
              ...
              {isAuthenticated ? (
                <Button disabled={!agree || isLoading} onClick={() => setConfirmOpen(true)} className="w-full shadow-md" size="lg" variant="gradient">
                  Buy Package
                </Button>
              ) : (
      ...
      {plan && (
        <Dialog open={confirmOpen} onOpenChange={(o: boolean) => { if (!isLoading) setConfirmOpen(o); }}>
